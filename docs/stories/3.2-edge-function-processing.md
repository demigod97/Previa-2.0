...
- [ ] **Task 1: Create Edge Function Structure** (AC: 1)
  - [ ] 1.1: Create `supabase/functions/process-document/index.ts`
  - [ ] 1.2: Set up Deno TypeScript configuration
  - [ ] 1.3: Import required Supabase client utilities
  - [ ] 1.4: Define request/response types
  - [ ] 1.5: Implement main request handler with CORS headers

- [ ] **Task 2: JWT Authentication** (AC: 1, 5)
  - [ ] 2.1: Extract JWT from `Authorization` header
  - [ ] 2.2: Verify JWT using Supabase Auth
  - [ ] 2.3: Extract user_id from JWT claims
  - [ ] 2.4: Return 401 if token invalid or missing
  - [ ] 2.5: Validate user_id matches payload

- [ ] **Task 3: Payload Validation** (AC: 1, 5)
  - [ ] 3.1: Check required fields: document_id, document_type, file_path, storage_bucket
  - [ ] 3.2: Validate document_type enum ('bank_statement' | 'receipt')
  - [ ] 3.3: Return 400 Bad Request with field-specific error messages
  - [ ] 3.4: Log validation errors

- [ ] **Task 4: Generate Signed URL** (AC: 2)
  - [ ] 4.1: Initialize Supabase Storage client with service role key
  - [ ] 4.2: Call `storage.from(bucket).createSignedUrl(file_path, 3600)`
  - [ ] 4.3: Handle errors if file not found or bucket invalid
  - [ ] 4.4: Return signed URL with 1-hour expiry

- [ ] **Task 5: Trigger n8n Workflow** (AC: 3, 4)
  - [ ] 5.1: Read N8N_WEBHOOK_URL from environment variables
  - [ ] 5.2: Construct webhook payload with document_id, document_type, file_url, user_id
  - [ ] 5.3: Add N8N_API_KEY to request headers
  - [ ] 5.4: Implement fetch with retry logic (3 attempts, exponential backoff)
  - [ ] 5.5: Update database status to 'processing' on success
  - [ ] 5.6: Update database status to 'failed' on failure

- [ ] **Task 6: Database Updates** (AC: 4)
  - [ ] 6.1: For bank_statements: update processing_status, processing_started_at
  - [ ] 6.2: For receipts: update processing_status, processing_started_at
  - [ ] 6.3: Use service role key for database updates (bypass RLS)
  - [ ] 6.4: Log database operation results

- [ ] **Task 7: Response Handling** (AC: 1, 5)
  - [ ] 7.1: Return 202 Accepted on successful trigger
  - [ ] 7.2: Return JSON: `{ success: true, job_id: document_id, message: 'Processing started' }`
  - [ ] 7.3: Return appropriate error codes (400, 401, 500)
  - [ ] 7.4: Include error details in response body

- [ ] **Task 8: Environment Setup & Testing** (AC: 6)
  - [ ] 8.1: Create `.env.local` file for Edge Function
  - [ ] 8.2: Add N8N_WEBHOOK_URL, N8N_API_KEY to Supabase secrets
  - [ ] 8.3: Test Edge Function locally with `supabase functions serve`
  - [ ] 8.4: Deploy to Supabase with `supabase functions deploy process-document`
  - [ ] 8.5: Integration test with real n8n workflow
  - [ ] 8.6: Unit test retry logic with mock failures
